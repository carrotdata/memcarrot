...
public class TestCommandParser extends TestParserBase{
  ...
  private void equalsKeys(String[] keys, RetrievalCommand c) {
    ...
    for (int i = 0; i < c.keys.length; i++) {
      ...
      assertTrue (com.onecache.core.util.Utils.compareTo(k, 0, k.length, c.keys[i], c.keySizes[i]) == 0);
    }
  }
  
  private void equalsKeys(byte[] key, AbstractMemcachedCommand c) {
    assertTrue (c.keyPtr > 0);
    assertTrue (com.onecache.core.util.Utils.compareTo(key, 0, key.length, c.keyPtr, c.keySize) == 0);
  }

  
  private void equalsValues(byte[] value, StorageCommand c) {
    assertTrue (c.valPtr > 0);
    assertTrue (com.onecache.core.util.Utils.compareTo(value, 0, value.length, c.valPtr, c.valSize) == 0);
  }
